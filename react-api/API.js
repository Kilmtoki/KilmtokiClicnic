...
app.get('/question/get', async (req, res) => {
  try {
    ...
    // Decode ข้อความภาษาไทยจาก Unicode escape sequence
    const thaiUsers = users.map(user => {
      user.Message = user.Message.decode('unicode-escape');
      return user;
    });

    res.status(200).send(thaiUsers);
  } catch (error) {
    console.error("Error in /users route:", error);
    res.status(500).send({
      "status": 'error',
